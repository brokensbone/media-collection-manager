...
describe("Guide", () => {
	...
	it("explains the loop and that it is not a recommender", () => {
		render(<Guide />);
		expect(
			screen.getByText(/saved → decide → wanted → acquire → owned/),
		).toBeTruthy();
	...
	it("covers reconnecting to Spotify and why it matters", () => {
		render(<Guide />);
		expect(
			screen.getByRole("heading", { name: /Staying connected to Spotify/ }),
		).toBeTruthy();
		expect(
			screen.getByText(/every worklist quietly stops filling/),
		).toBeTruthy();
	});
});
