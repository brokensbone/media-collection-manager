...
describe('Tasks', () => {
  ...
  it('fetches from /imports/tasks and offers an archive link', async () => {
    const fetchMock = mockApi([item({ id: 1, name: 'Done', state: 'imported' })])
    render(<Tasks />)
    await screen.findByText('Done')
    expect(fetchMock.mock.calls[0][0]).toBe('/imports/tasks')
    expect(screen.getByRole('button', { name: /View completed archive/ })).toBeTruthy()
  })
  ...
  it('in archive mode fetches /imports/archive and shows a back link', async () => {
    const fetchMock = mockApi([item({ id: 1, name: 'Old', state: 'imported' })])
