    ...
    <select
      ...
      onChange={(e) => {
        fetch(`/imports/${item.id}/classify`, {
          ...
          body: JSON.stringify({ kind: kindTarget(e.target.value as ManualImportKind) }),
        }).then(onChange)
      }}
    >
...
function kindValue(item: ImportItem): ManualImportKind | '' {
  if (item.import_target === 'review' && item.media_kind === 'unknown') return ''
  if (item.import_target === 'workspace' || item.media_kind === 'workspace') return 'workspace'
  if (item.import_target === 'tv' || item.media_kind === 'tv') return 'tv'
  if (item.import_target === 'film' || item.media_kind === 'film') return 'film'
  return 'music'
}

function kindTarget(kind: ManualImportKind): 'beets' | 'tv' | 'film' | 'workspace' {
